...
function Beautify(value,floats)
{
	...
	var output=(value.toString().indexOf('e+')!=-1 && Game.prefs.format==1)?value.toPrecision(3).toString():formatter(value).toString();
	if(Game.prefs.format || !Game.prefs.formatlang){
		...
		if(output.indexOf('.') == -1){
			...
			output = output.split('.')[0].replace(/\B(?=(\d{3})+(?!\d))/g,',')+'.'+output.split('.')[1];
		}
	}
...
Game.Launch=function()
{
	...
	'<div class="listing">トップバーの宣伝部分と別バージョン欄につきましては情報を変更してしまわないよう、可能な限り原文に即して訳しています。また、日本語版のため、システムの中で、単位を千進法から万進法に変更、ベーカリー名の2バイト文字等への対応(2バイト文字の使用は自己責任でお願いします)をしています。また、英語と日本語による数の単位の差から日本語版では一部仏典の単位、上数法を使用しており、本家では10の300乗のところを10の444乗まで短縮表記ができるようになっております。これらの変更について公式から指摘された場合、該当部分の変更を戻します。</div>'+
	...
	'</div><div class="subsection update small">'+
	'<div class="title">2020/08/08 - 当座預金口座(ベータ版)</div>'+
	...
	'<div class="title">2020/06/18 - 一気に荒稼ぎ(ベータ版)</div>'+
	...
	Game.Init=function()
	{
		...
		//if you're making a mod, don't forget to add a Game.Win('サードパーティ') somewhere in there!
		...
		Game.bakeryNameL.innerHTML=Game.bakeryName+'のベーカリー';
		...
		Game.attachTooltip(l('topbarMobileCC'),'<div style="padding:8px;width:250px;text-align:center;">スマホでCookieClickerを遊ぼう!<br>(現在ベータ、Androidだけです。iOSバージョンは後ほど)</div>','this');
		...
		Game.lumpTooltip=function()
		{
			...
			str+='<div class="line"></div>';
			if (age<Game.lumpMatureAge) str+='この角砂糖は成長中で、熟すまで<b>'+Game.sayTime(((Game.lumpMatureAge-age)/1000+1)*Game.fps,-1)+'</b>かかります。';
		...
		Game.mouseCps=function()
		{
			...
			if (Game.Has('万手観音')) add+=		0.5;
			if (Game.Has('億手観音')) add+=		5;
			if (Game.Has('兆手観音')) add+=		50;
			if (Game.Has('京手観音')) add+=	500;
			if (Game.Has('垓手観音')) add+=	5000;
			if (Game.Has('秭手観音')) add+=	50000;
			if (Game.Has('穣手観音')) add+=	500000;
			if (Game.Has('溝手観音')) add+=	5000000;
			if (Game.Has('澗手観音')) add+=	50000000;
			var num=0;
			...
			if (Game.Has('プラズマ大理石のマウス')) add+=Game.cookiesPs*0.01;
			if (Game.Has('奇石から生まれたマウス')) add+=Game.cookiesPs*0.01;
		...
		Game.dropRateMult=function()
		{
			...
			if (Game.Has('ドラゴンのぬいぐるみ')) rate*=1.03;
		...
		Game.UpdateMenu=function()
		{
			...
			else if (Game.onMenu=='log')
			{
				...
				if (!Game.HasAchiev('昔の日々')) str+='<div style="text-align:right;width:100%;"><div '+Game.clickStr+'="Game.SparkleAt(Game.mouseX,Game.mouseY);PlaySound(\'snd/tick.mp3\');PlaySound(\'snd/shimmerClick.mp3\');Game.Win(\'昔の日々\');Game.UpdateMenu();" class="icon" style="display:inline-block;transform:scale(0.5);cursor:pointer;width:48px;height:48px;background-position:'+(-12*48)+'px '+(-3*48)+'px;"></div></div>';
			}
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || Game.cookiesEarned>=10100000000)
			{
				...
				if (Math.random()<0.75 || Game.cookiesEarned<10000)
				{
					...
					'ニュース : 凶暴なウサギの群れにより、数十億円の物的被害。対策として新たなミクソマトーシス(兎粘液腫)ウィルス株が開発された。',
				...
				if (Math.random()<0.05)
				{
					...
					if (Game.Has('役員猫')) list.push('News : kittens strutting around in hot little business suits shouting cut-throat orders at their assistants, possibly the cutest thing this reporter has ever seen!');
					if (Game.Has('子猫の天使')) list.push('ニュース : 「目に見えぬ猫が耳の中でゴロゴロ鳴いていても無視すること」科学者が警告。「人生の選択を誤らせようとする罠だ」');
					if (Game.Has('猫賃')) list.push('News : kittens break glass ceiling! Do they have any idea how expensive those are!');
					if (Game.HasAchiev('ジェリクル')) list.push('News : local kittens involved in misguided musical production, leave audience perturbed and unnerved.');
				}
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			this.rebuild=function()
			{
				...
				*/
				...
				l('productLevel'+me.id).innerHTML='Lv.'+Beautify(me.level);
				if (Game.isMinigameReady(me) && Game.ascensionMode!=1)
				{
					l('productMinigameButton'+me.id).style.display='block';
					if (!me.onMinigame) l('productMinigameButton'+me.id).innerHTML='展開 '+me.minigameName;
					else l('productMinigameButton'+me.id).innerHTML='縮小 '+me.minigameName;
				}
		...
		new Game.Object('カーソル','カーソル|カーソル|をクリックした|[X]本の追加の指が|[X]本の追加の指が','10秒毎に自動クリックします。',0,0,{},15,function(me){
			...
			if (this.amount>=1) Game.Win('クリック');if (this.amount>=2) Game.Win('ダブルクリック');if (this.amount>=50) Game.Win('マウスホイール');if (this.amount>=100) Game.Win('二十日鼠と人間');if (this.amount>=200) Game.Win('指人間');if (this.amount>=300) Game.Win('超過多指症');if (this.amount>=400) Game.Win('ドクター・T');if (this.amount>=500) Game.Win('親指、指骨、中手骨');if (this.amount>=600) Game.Win('人差し指と親指で');if (this.amount>=700) Game.Win('尊敬するよ');
		});
		...
		Game.foolObjects={
			...
			'Javascriptコンソール':{name:'主義',desc:'-宗教、文化、哲学-などの多くの形態をとると、教義は適切に処理された場合、文明に永続的な影響を与え、心と人々を作り直し、すべての未来の世代が-より多くのクッキーの生産と獲得-という共通の目標を共有するようになります。',icon:17},
		};
		...
		Game.Upgrade.prototype.getPrice=function()
		{
			...
			if (this.pool!='prestige')
			{
				...
				if (Game.Has('幸運 No.100')) price*=0.99;
				if (this.kitten && Game.Has('猫賃')) price*=0.9;
		...
		new Game.Upgrade('万手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+0.5</b> クッキーされる。<q>クリッククリックなう</q>',10000000,[0,14]);Game.MakeTiered(Game.last,5,0);
		new Game.Upgrade('億手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+5</b> クッキーされる。<q>クリッククリッククリックなう</q>',100000000,[0,15]);Game.MakeTiered(Game.last,6,0);
		new Game.Upgrade('兆手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+50</b> クッキーされる。<q>クリッククリッククリッククリックなう</q>',1000000000,[0,16]);Game.MakeTiered(Game.last,7,0);
		...
		Game.NewUpgradeCookie({name:'ピーナッツバタークッキー',desc:'ジャムクッキーはお好みでどうぞ！ ',icon:[1,3],power:								1,	price:	9999999*5});
		...
		Game.NewUpgradeCookie({name:'マカダミアナッツクッキー',desc:'くっそウマい!',icon:[5,3],power:								2,	price:	999999999});
		...
		order=100;
		new Game.Upgrade('京手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+500</b> クッキーされる。<q>クリッククリッククリッククリッククリック</q>',10000000000,[0,17]);Game.MakeTiered(Game.last,8,0);
		...
		order=10020;
		Game.NewUpgradeCookie({name:'ダークチョコレートコーティングクッキー',desc:'このクッキーは光を良く吸収するので、見つけるためにはほとんどの場合、目を凝らす必要がある。',icon:[10,3],power:			4,	price:	99999999999});
		Game.NewUpgradeCookie({name:'ホワイトチョコレートコーティングクッキー',desc:'フレーバーで完璧にコーティングされてて眩しいくらいピカピカだ。',icon:[11,3],power:					4,	price:	99999999999});
		...
		order=100;
		new Game.Upgrade('垓手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+5000</b> クッキーされる。<q>お前は, ただクリックしてクリックしてクリックしてクリックしてクリックするだけだ。本当に簡単だろう?</q>',10000000000000,[0,18]);Game.MakeTiered(Game.last,9,0);
		...
		order=100;
		new Game.Upgrade('秭手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+50000</b> クッキーされる。<q>時々<br>ただ<br>クリックするもの</q>',10000000000000000,[0,19]);Game.MakeTiered(Game.last,10,0);
		...
		order=100;
		new Game.Upgrade('穣手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+500000</b> クッキーされる。<q>[ここにフレーバーテキストを挿入]</q>',10000000000000000000,[12,20]);Game.MakeTiered(Game.last,11,0);
		new Game.Upgrade('溝手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+5000000</b> クッキーされる。<q>指は添えるだけ</q>',10000000000000000000000,[12,19]);Game.MakeTiered(Game.last,12,0);
		...
		order=10020;
		Game.NewUpgradeCookie({name:'ピュアブラックチョコレートクッキー',desc:'ラボ製の最も暗いココアより暗い物質に漬け込んだクッキー(『チョコアレート』と名付けられている)。',icon:[26,3],power:									4,price: 9999999999999999*5});
		Game.NewUpgradeCookie({name:'ピュアホワイトチョコレートクッキー',desc:'緻密なこのビスケットのコーティングさえあれば、真っ暗な環境で光を屈折させることができる。',icon:[26,4],power:	4,price: 9999999999999999*5});
		...
		new Game.Upgrade('アップグレードスロット II',desc,	2000,[1,10]);Game.last.pool='prestige';Game.last.parents=['アップグレードスロット I'];Game.last.iconFunction=function(){return Game.PermanentSlotIcon(1);};Game.last.activateFunction=function(){Game.AssignPermanentSlot(1);};
		new Game.Upgrade('アップグレードスロット III',desc,	30000,[2,10]);Game.last.pool='prestige';Game.last.parents=['アップグレードスロット II'];Game.last.iconFunction=function(){return Game.PermanentSlotIcon(2);};Game.last.activateFunction=function(){Game.AssignPermanentSlot(2);};
		new Game.Upgrade('アップグレードスロット IV',desc,	400000,[3,10]);Game.last.pool='prestige';Game.last.parents=['アップグレードスロット III'];Game.last.iconFunction=function(){return Game.PermanentSlotIcon(3);};Game.last.activateFunction=function(){Game.AssignPermanentSlot(3);};
		new Game.Upgrade('アップグレードスロット V',desc,	5000000,[4,10]);Game.last.pool='prestige';Game.last.parents=['アップグレードスロット IV'];Game.last.iconFunction=function(){return Game.PermanentSlotIcon(4);};Game.last.activateFunction=function(){Game.AssignPermanentSlot(4);};
		...
		order=10300;
		Game.NewUpgradeCookie({name:'ミルクチョコレートバタービスケット',desc:'全ての施設を100個所有した報酬です。<br>偉大な起業家が彫られています。',icon:[27,8],power:	10,price: 999999999999999999999,locked:1});
		Game.NewUpgradeCookie({name:'ダークチョコレートバタービスケット',desc:'全ての施設を150個所有した報酬です。<br>経験豊富なクッキー界の大物の像で飾られています。',icon:[27,9],power:	10,price: 999999999999999999999999,locked:1});
		Game.NewUpgradeCookie({name:'ホワイトチョコレートバタービスケット',desc:'全ての施設を200個所有した報酬です。<br>チョコレートには、堂々たる社交界の大御所が彫られています。',icon:[28,9],power:	10,price: 999999999999999999999999999,locked:1});
		Game.NewUpgradeCookie({name:'ルビーチョコレートバタービスケット',desc:'全ての施設を250個所有した報酬です。<br>希少な赤チョコレートで覆われています。このビスケットには、力に狂い己を見失ったクッキー実業家の顔を表現したエッチングが施されています。',icon:[28,8],power:	10,price: 999999999999999999999999999999,locked:1});
		...
		order=10300;
		Game.NewUpgradeCookie({name:'ラベンダーチョコレートバタービスケット',desc:'全ての施設を300個所有した報酬です。<br>この捉え難い風味のビスケットは、数十年に渡る極秘研究の成果の象徴です。チョコレートに象られた肖像は、いにしえの菓子焼き道に己の全てを捧げた著名な起業家に酷似しています。',icon:[26,10],power:	10,price: 999999999999999999999999999999999,locked:1});
		...
		new Game.Upgrade('プラズマ大理石のマウス','クリックによるクッキー取得量が <b>CpSの1%分</b> 増加する。<q>視界の隅がブレてぼやけてくるこのマウスは、ほんのそよ風が掠めただけでクリックの嵐を巻き起こす。</q>',500000000000000000000000000,[11,30]);Game.MakeTiered(Game.last,12,11);
		...
		order=10300;
		Game.NewUpgradeCookie({name:'合成チョコレート翠緑蜂蜜バタービスケット',desc:'全ての施設を350個所有した報酬です。<br>このバタービスケットの製法は、かつては、とある古い山上修道院だけに伝わる門外不出の遺産でした。その味はこの上なく上品で、完全な無味無臭に特別に設計加工されたラボ製の板チョコレートでなければ、この完璧なフレーバーの再現は不可能でしょう。<br>更に、このビスケットにはあなたの顔が彫ってあります。',icon:[24,26],power:	10,price: 999999999999999999999999999999999999,locked:1});
		Game.NewUpgradeCookie({name:'ロイヤルラズベリーチョコレートバタービスケット',desc:'全ての施設を400個所有した報酬です。<br>かつては誇大妄想狂の権力者にだけ提供されていた、このフルーティチョコレート独自の特性として、他には全く無い風味と舌触りがあります。その法外な価値が、チョコレートの表面に描かれたあなたの肖像画によって上がるのか下がるのか…今はまだ不明です。',icon:[25,26],power:	10,price: 999999999999999999999999999999999999999,locked:1});
		Game.NewUpgradeCookie({name:'超凝縮高エネルギーチョコレートバタービスケット',desc:'全ての施設を450個所有した報酬です。<br>水素爆弾2、3発分のパワーを、この一枚に注ぎ込みました。原子力技術者および株主ほぼ全員を困惑させた経緯を通じつつも。目下、国際会議の中心でかなり白熱した論議が展開中です。このチョコレートに関し更に説明しようものなら、幾つかの国家の機密に抵触する恐れがあります。しかし、一つだけ加えるなら、それには誰かの肖像が描かれているようです。恐らくは、あなたの?',icon:[26,26],power:	10,price: 999999999999999999999999999999999999999999,locked:1});
		...
		new Game.Upgrade('思いつきで作ったチェックリスト','<b>全買い</b>の機能を解禁し、お店でアップグレードの即時一括購入できるようになる(安い順に)。<br>また、<b>保管庫</b>も解禁され、自動購入されたくないアップグレードが並べられるお店の一部となる。<q>お洒落なグランマのアクセサリー?はいチェック。超越次元の忌まわしきもの?はいチェック。なーんとなく卵いろいろ?はいチェック。「ピーン」って鳴る機械?はいどんどんチェック。 </q>',900000,[28,26]);Game.last.pool='prestige';Game.last.parents=['消せない記憶','アップグレードスロット IV'];
		
		order=10300;
		Game.NewUpgradeCookie({name:'純然たる漆黒のチョコレートバタービスケット',desc:'全ての施設を500個所有した報酬です。<br>このチョコレートは高純度で瑕疵一つすら無い為、それ自身は色を持たないものの、代わりに周辺物の外観を何でも映し込んでしまいます。表面が完璧なまでに滑らか(ピコメートル単位)なので、あなたは自身の肖像の刻印が無いと気付き、些か驚きました - が、程なく自分の顔が寸分違わず鏡の如く反映されているのを深く理解するでしょう。 ',icon:[24,27],power:	10,price: 999999999999999999999999999999999999999999999,locked:1});
		...
		new Game.Upgrade('宇宙レベルのビギナーズラック','周回内において <b>ヘブンリーチップスの極意</b> の購入以前に、アイテムドロップ率を <b>5倍出やすく</b> する。<q>Oh! A penny!<br>Oh! A priceless heirloom!<br>Oh! Another penny!</q>',999999999*15,[8,10]);Game.last.pool='prestige';Game.last.parents=['煌めくベール'];
		new Game.Upgrade('強化された薄膜','<b>煌めくベール</b> の耐久性が上がり、<b>10%の確率</b> で壊れなくなる。また、CpSが <b>+10%</b> される。<q>A consistency between jellyfish and cling wrap.</q>',999999999*15,[7,10]);Game.last.pool='prestige';Game.last.parents=['煌めくベール'];
		...
		order=10060;
		Game.NewUpgradeCookie({name:'ピザ',desc:'ピザ、でもあふれんばかりのトマトとチーズのアイシングでつや消ししてもいない、クッキーほどの歯ごたえもないピザだったら?結局クッキーじゃない?それがどうした。',icon:[31,9],require:'クッキーに非ざるものの箱入りセット',		power:5,price: Math.pow(10,44)});
		...
		Game.NewUpgradeCookie({name:'ティムダム',desc:'チョコレートでつなげられ、さらにそこから大量のチョコでコーティングされた2つのビスケット。<br>あなたはこう疑問を浮かべる - どっちがディムで、どっちがダムか?',icon:[31,10],require:'ブランドビスケットの箱入りセット',power:												2,	price:	999999999999999999999999999999*5});
		
		order=10060;
		Game.NewUpgradeCookie({name:'キャンディー',desc:'お菓子の世界に存在する大きなふたつの柱 : もちろん、クッキー - あとキャンディー。<br>キャンディーだけでまったく新しいゲームを作ることも可能ですが、とりあえず今のところはこれらのごく普通なキャンディーの盛り合わせをお楽しみください。',icon:[30,10],require:'クッキーに非ざるものの箱入りセット',		power:5,price: Math.pow(10,46)});
		
		
		order=19000;
		new Game.TieredUpgrade('幸運 No.001','カーソルが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Fingers are not the only thing you can count on.</q>','カーソル','fortune');
		new Game.TieredUpgrade('幸運 No.002','グランマが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>A wrinkle is a crack in a mundane facade.</q>','グランマ','fortune');
		new Game.TieredUpgrade('幸運 No.003','農場が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>The seeds of tomorrow already lie within the seeds of today.</q>','農場','fortune');
		new Game.TieredUpgrade('幸運 No.004','鉱山が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Riches from deep under elevate you all the same.</q>','鉱山','fortune');
		new Game.TieredUpgrade('幸運 No.005','工場が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>True worth is not in what you find, but in what you make.</q>','工場','fortune');
		new Game.TieredUpgrade('幸運 No.006','銀行が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>The value of money means nothing to a pocket.</q>','銀行','fortune');
		new Game.TieredUpgrade('幸運 No.007','神殿が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Not all guides deserve worship.</q>','神殿','fortune');
		new Game.TieredUpgrade('幸運 No.008','魔法使いの塔が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Magic is about two things - showmanship, and rabbits.</q>','魔法使いの塔','fortune');
		new Game.TieredUpgrade('幸運 No.009','宇宙船が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Every mile travelled expands the mind by just as much.</q>','宇宙船','fortune');
		new Game.TieredUpgrade('幸運 No.010','錬金術室が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Don\'t get used to yourself. You\'re gonna have to change.</q>','錬金術室','fortune');
		new Game.TieredUpgrade('幸運 No.011','ポータルが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Every doorway is a gamble. Tread with care.</q>','ポータル','fortune');
		new Game.TieredUpgrade('幸運 No.012','タイムマシンが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Do your future self a favor; they\'ll thank you for it.</q>','タイムマシン','fortune');
		new Game.TieredUpgrade('幸運 No.013','反物質凝縮器が <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>The world is made of what we put into it.</q>','反物質凝縮器','fortune');
		new Game.TieredUpgrade('幸運 No.014','プリズムが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Staring at a dazzling light can blind you back to darkness.</q>','プリズム','fortune');
		new Game.TieredUpgrade('幸運 No.015','チャンスメーカーが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>Don\'t leave to blind chance what you could accomplish with deaf skill.</q>','チャンスメーカー','fortune');
		new Game.TieredUpgrade('幸運 No.016','自己無限生成エンジンが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>It\'s good to see yourself in others. Remember to see yourself in yourself, too.</q>','自己無限生成エンジン','fortune');
		new Game.TieredUpgrade('幸運 No.017','Javascriptコンソールが <b>7%</b> より効果を発揮し、 <b>7%</b> 安くなる。<q>If things aren\'t working out for you, rewrite the rules.</q>','Javascriptコンソール','fortune');
		
		
		order=19100;
		//note : price for these capped to base price OR 1 day of unbuffed CpS
		new Game.Upgrade('幸運 No.100','アップグレードと施設の価格が <b>-1%</b> され、CpSが <b>+1%</b> される。<q>True wealth is counted in gifts.</q>',
		...
		new Game.Upgrade('幸運 No.101','CpSが <b>+7%</b> される。<q>Some people dream of fortunes; others dream of cookies.</q>',Game.Tiers['fortune'].price*100000000,[0,0]);Game.MakeTiered(Game.last,'fortune',10);
		Game.last.priceFunc=function(me){return Math.min(me.basePrice,Game.unbuffedCps*60*60*24);}
		new Game.Upgrade('幸運 No.102','ゲームを閉じている間のCpSが <b>+1%</b> される<small>(天国系アップグレードの超絶の双門を所有しているならば)</small>。<q>Help, I\'m trapped in a browser game!</q>',Game.Tiers['fortune'].price*100000000000,[0,0]);Game.MakeTiered(Game.last,'fortune',10);
		Game.last.priceFunc=function(me){return Math.min(me.basePrice,Game.unbuffedCps*60*60*24);}
		new Game.Upgrade('幸運 No.103','より多くのミルクを持っているほど<b>よりCpSが多く</b>上昇する。<q>Don\'t believe the superstitions; all cats are good luck.</q>',Game.Tiers['fortune'].price*100000000000000,[0,0]);Game.MakeTiered(Game.last,'fortune',18);Game.last.kitten=1;
		Game.last.priceFunc=function(me){return Math.min(me.basePrice,Game.unbuffedCps*60*60*24);}
		new Game.Upgrade('幸運 No.104','クリックによるクッキー取得量が <b>CpSの1%分</b> 増加する。<q>Remember to stay in touch.</q>',Game.Tiers['fortune'].price*100000000000,[0,0]);Game.MakeTiered(Game.last,'fortune',11);
		Game.last.priceFunc=function(me){return Math.min(me.basePrice,Game.unbuffedCps*60*60*24);}
		...
		order=10300;
		Game.NewUpgradeCookie({name:'プリズムのハートのビスケット',desc:'An every-flavor biscuit that stands for universal love and being true to yourself.',require:'永久のハートのビスケット',season:'valentines',icon:[30,8],							power:heartPower,price: 1000000000000000000000000});Game.last.order=10300.175;
		
		order=19100;
		new Game.Upgrade('猫賃','賢明な経理によって、なんと猫のアップグレードを <b>10%安く</b> する。<q>Cats can have little a salary.<br>Cats are expert hagglers and have a keen sense of bargaining, especially in the case of cash.</q>',9000000000,[31,8]);Game.last.pool='prestige';Game.last.parents=['子猫の天使'];Game.last.kitten=1;
		new Game.Upgrade('ドラゴン飼育','一度孵ったときにクリックしてあげることで<b>マイドラゴンを飼う</b>能力を解禁する。<q>Dragons do not purr. If your dragon starts purring, vacate the area immediately.</q>',99999999999,[30,12]);Game.last.pool='prestige';Game.last.parents=['ドラゴンの焼き方','残された幸運'];
		
		order=25100;
		var dragonDropUpgradeCost=function(me){return Game.unbuffedCps*60*30*((Game.dragonLevel<Game.dragonLevels.length-1)?1:0.1);};
		new Game.Upgrade('ドラゴンの鱗','クッキーの生産倍率が <b>+3%</b> される。<br>費用はCpSに依存するが、十分に鍛えられたドラゴンがいれば 10分の1 になる。<q>Your dragon sheds these regularly, so this one probably won\'t be missed.<br>Note: icon not to scale.</q>',999,[30,14]);Game.last.priceFunc=dragonDropUpgradeCost;
		new Game.Upgrade('ドラゴンの爪','クリックが <b>+3%強力</b> になる。<br>費用はCpSに依存するが、十分に鍛えられたドラゴンがいれば 10分の1 になる。<q>Will grow back in a few days\' time.<br>A six-inch retractable claw, like a razor, from the middle toe. So you know, try to show a little respect.</q>',999,[31,14]);Game.last.priceFunc=dragonDropUpgradeCost;
		new Game.Upgrade('ドラゴンの牙','ゴールデンクッキーの入手クッキーが <b>+3%</b> される。<br>刈れよドラゴン と ドラゴン便 が <b>10%強力</b> になる。<br>費用はCpSに依存するが、十分に鍛えられたドラゴンがいれば 10分の1 になる。<q>Just a fallen baby tooth your dragon wanted you to have, as a gift.<br>It might be smaller than an adult tooth, but it\'s still frighteningly sharp - and displays some awe-inspiring cavities, which you might expect from a creature made out of sweets.</q>',999,[30,15]);Game.last.priceFunc=dragonDropUpgradeCost;
		new Game.Upgrade('ドラゴンのぬいぐるみ','アイテムのドロップ率が <b>+3%</b> される。<br>費用はCpSに依存するが、十分に鍛えられたドラゴンがいれば 10分の1 になる。<q>Your dragon used to sleep with this. Now it\'s time for you to have it.<br>Crafted in the likelihood of a fearsome beast. Stuffed with magical herbs picked long ago by a wandering wizard. Woven from elven yarn and a polyester blend.</q>',999,[31,15]);Game.last.priceFunc=dragonDropUpgradeCost;
		
		order=10020;
		Game.NewUpgradeCookie({name:'グラノーラクッキー',desc:'Wait! These are just oatmeal cookies mixed with raisin cookies! What next, half-dark chocolate half-white chocolate cookies?',icon:[28,32],power:						5,price: getCookiePrice(28)});
		Game.NewUpgradeCookie({name:'リコッタクッキー',desc:'Light and cake-like. Often flavored with lemon or almond extract. Sprinkles optional. Allegedly Italian. Investigation pending.',icon:[29,32],power:						5,price: getCookiePrice(29)});
		Game.NewUpgradeCookie({name:'ピンクケーキ',desc:'The icing on these Dutch cookies is traditionally pink, but different colors may be used for special occasions - such as pink to celebrate Breast Cancer Awareness Month, or for International Flamingo Day, pink.',icon:[30,32],power:						5,price: getCookiePrice(30)});
		Game.NewUpgradeCookie({name:'ピーナッツバター入りカップクッキー',desc:'What more poignant example of modern societal struggles than the brazen reclaiming of a corporate product by integrating it in the vastly more authentic shell of a homemade undertaking? Anyway this is a peanut butter cup, baked into a cookie. It\'s pretty good!',icon:[31,32],power:						5,price: getCookiePrice(31)});
		Game.NewUpgradeCookie({name:'セサミクッキー',desc:'Look at all the little seeds on these! It\'s like someone dropped them on the street or something! A very welcoming and educational street!',icon:[22,33],power:						5,price: getCookiePrice(32)});
		Game.NewUpgradeCookie({name:'たい焼き',desc:'A pastry fish filled with red bean paste, doomed to live an existence of constant and excruciating pain as its aquatic environment slowly dissolves its soft doughy body.<br>Also comes in chocolate flavor!',icon:[23,33],power:						5,price: getCookiePrice(33)});
		Game.NewUpgradeCookie({name:'バニレキプフェル',desc:'Nut-based cookies from Central Europe, coated in powdered vanilla sugar. Regular kipferl, crescent-shaped bread rolls from the same region, are much less exciting.',icon:[24,33],power:						5,price: getCookiePrice(34)});
		
		order=10300;
		Game.NewUpgradeCookie({name:'宇宙を秘めたチョコレートバタービスケット',desc:'Rewarded for owning 550 of everything.<br>Through some strange trick of magic or technology, looking at this cookie is like peering into a deep ocean of ancient stars. The origins of this biscuit are unknown; its manufacture, as far as your best investigators can tell, left no paper trail. From a certain angle, if you squint hard enough, you\'ll notice that a number of stars near the center are arranged to resemble the outline of your own face.',icon:[27,32],power:	10,price: 999999999999999999999999999999999999999999999999,locked:1});
		
		order=100;new Game.Upgrade('澗手観音','クリックとカーソルの生産量が所持しているカーソル以外の施設1つにつき <b>+50000000</b> クッキーされる。<q>Only for the freakiest handshakes.</q>',10000000000000000000000000,[12,31]);Game.MakeTiered(Game.last,13,0);
		order=150;new Game.Upgrade('奇石から生まれたマウス','クリックによるクッキー取得量が <b>CpSの1%分</b> 増加する。<q>Composed of a material that neither science nor philosophy are equipped to conceptualize. And boy, does it ever click.</q>',50000000000000000000000000000,[11,31]);Game.MakeTiered(Game.last,13,11);
		order=200;new Game.TieredUpgrade('変性発生','グランマが <b>2倍</b> 効率的になる。<q>Genetic testing shows that most of your grandmas are infected with a strange degenerative disease that only seems to further their powers; the more time passes, the older they get. This should concern you.</q>','グランマ',12);
		order=300;new Game.TieredUpgrade('世界種子貯蔵庫','農場が <b>2倍</b> 効率的になる。<q>An enormous genetic repository that could outlive an apocalypse. Guarantees the survival of your empire, or at the very least its agricultural components, should civilization fall. Which should be any day now.</q>','農場',12);
		order=400;new Game.TieredUpgrade('天掘り','鉱山が <b>2倍</b> 効率的になる。<q>You\'ve dug your drills through just about every solid surface you could find. But did you know recent advances have revealed untold riches hiding within non-solid surfaces too?</q>','鉱山',12);
		order=500;new Game.TieredUpgrade('行動の見直し','工場が <b>2倍</b> 効率的になる。<q>Through careful social engineering you\'ve convinced your workers that "union" is a slur that only the most vile and repugnant filth among us would ever dare utter! Sometimes progress isn\'t in the big machines, it\'s in the little lies!</q>','工場',12);
		order=525;new Game.TieredUpgrade('利他の連鎖','銀行が <b>2倍</b> 効率的になる。<q>You control so many branches of the global economy and legislative bodies that, through a particularly creative loophole, donating money (to yourself) grants you even more cash in tax deductions than you started with!</q>','銀行',12);
		order=550;new Game.TieredUpgrade('珍説','神殿が <b>2倍</b> 効率的になる。<q>You don\'t get rich starting a religion. If you want to get rich, you write science fiction.</q>','神殿',12);
		order=575;new Game.TieredUpgrade('魔蜂使い','魔法使いの塔が <b>2倍</b> 効率的になる。<q>You\'ve unleashed a swarm of magically-enhanced bees upon mankind! Their stinging spells may be the bane of all living things but you\'re certain you can put their delicious, purple, fizzy honey to good use!</q>','魔法使いの塔',12);
		order=600;new Game.TieredUpgrade('トーラス状宇宙','宇宙船が <b>2倍</b> 効率的になる。<q>If you think of the universe as an nth-dimensional torus that wraps back on itself in every direction, you can save a fortune on rocket fuel! Of course the universe isn\'t actually shaped like that, but you\'ve never let details stand in your way.</q>','宇宙船',12);
		order=700;new Game.TieredUpgrade('科学と錬金術の調和','錬金術室が <b>2倍</b> 効率的になる。<q>It\'s time for modern science and the mystical domains of the occult to work together at last. What do gravitons transmute into? What if alkahest is pH-neutral? Should a homunculus have the right to vote? And other exciting questions coming to you soon, whether you like it or not.</q>','錬金術室',12);
		order=800;new Game.TieredUpgrade('奴の到来','ポータルが <b>2倍</b> 効率的になる。<q>"He comes! He comes at last! Just like the prophecies foretold! And as He steps out of the portal, your engineers begin slicing Him into convenient chunks before transporting His writhing cosmic flesh to your factories, where He will be processed and converted into a new and exciting cookie flavor, available tomorrow.</q>','ポータル',12);
		order=900;new Game.TieredUpgrade('時間分割','タイムマシンが <b>2倍</b> 効率的になる。<q>Time is infinite, yes... But what if, nestled within each second, were even more infinities? Every moment an eternity! Think of how many scheduling troubles this solves!</q>','タイムマシン',12);
		order=1000;new Game.TieredUpgrade('自己味付け','反物質凝縮器が <b>2倍</b> 効率的になる。<q>Deep under the earth, in the most sterile laboratory, in the most vast and expensive particle accelerator ever devised, your scientists have synthesized -for a fraction of a second- the physical manifestation of pure flavor. Highly unstable, and gone in a puff of radioactive energy, it nonetheless left your team shivering with awe... and hunger.</q>','反物質凝縮器',12);
		order=1100;new Game.TieredUpgrade('光速度限界','プリズムが <b>2倍</b> 効率的になる。<q>Whoah, slow down. Harvesting light is well and good but it\'d be much easier if it weren\'t so dang fast! This should thankfully take care of that.</q>','プリズム',12);
		order=1200;new Game.TieredUpgrade('ちょっとした決定論','チャンスメーカーが <b>2倍</b> 効率的になる。<q>By knowing the exact position and movement of every particle in the universe, you\'re able to predict everything that can ever happen, leaving nothing to chance. This was a doozy to pull off mind you, but it\'s helped you win 50 bucks at the horse races so you could say it\'s already paying off.</q>','チャンスメーカー',12);
		order=1300;new Game.TieredUpgrade('このアップグレード','自己無限生成エンジンが <b>2倍</b> 効率的になる。<q>This upgrade\'s flavor text likes to refer to itself, as well as to the fact that it likes to refer to itself. You should really buy this upgrade before it starts doing anything more obnoxious.</q>','自己無限生成エンジン',12);
		order=1400;new Game.TieredUpgrade('最大級冷却ファン','Javascriptコンソールが <b>2倍</b> 効率的になる。<q>Let\'s face it, baking cookies isn\'t the most optimized thing there is. So you\'ve purchased your biggest fans yet and stuck them next to your computers to keep things chill and in working order. Cool!</q>','Javascriptコンソール',12);
		
		
		order=10020;
		Game.NewUpgradeCookie({name:'バッテンバーグビスケット',desc:'Inspired by a cake of the same name, itself named after a prince of the same name. You suppose you could play a really, really short game of chess on these.',icon:[28,33],power:						5,price: getCookiePrice(35)});
		Game.NewUpgradeCookie({name:'ロゼットクッキー',desc:'Intricate fried pastries from Northern Europe, made using specialized irons and dipped in icing sugar. While usually eaten as a delicious treat, these are often also used as Christmas tree decorations, or worn elegantly on one\'s lapel to symbolize the nah I\'m just messing with you.',icon:[26,33],power:						5,price: getCookiePrice(36)});
		Game.NewUpgradeCookie({name:'ギャングメーカー',desc:'The little bit of raspberry jam at its center is crucial; a plain butter cookie with chocolate topping does not a gangmaker make.',icon:[27,33],power:						5,price: getCookiePrice(37)});
		Game.NewUpgradeCookie({name:'ウェルシュケーキ',desc:'Welsh cookies, also known as Welsh cakes, bakestones, griddle cakes, griddle scones, or pics, or in Welsh: <i>picau ar y maen, pice bach, cacennau cri</i> or <i>teisennau gradell</i>, are rich currant-filled scone-like biscuits of uncertain origin.',icon:[29,33],power:						5,price: getCookiePrice(38)});
		Game.NewUpgradeCookie({name:'ラズベリーチーズケーキクッキー',desc:'The humble raspberry cheesecake, now in ascended cookie form. Researchers posit that raspberry cheesecake cookies are evidence that the final form of every baked good, through convergent evolution, approaches that of a cookie, in a process known as cookienisation.',icon:[25,33],power:						5,price: getCookiePrice(39)});
		...
		order=1050;
		new Game.Achievement('親指、指骨、中手骨','<b>500</b> カーソル所持する。<q>とナックルズ</q>',[0,15]);
		
		order=6000;
		new Game.Achievement('博学','アップグレードを <b>300</b> 個、施設を <b>3000</b> 個所持する。<q>ローマは一日にして成らず…普通は何日かかかるものさ。</q>',[29,7]);
		...
		order=2200;
		Game.TieredAchievement('自己完結型','<b>1</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',1);
		Game.TieredAchievement('延々仰天','<b>50</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',2);
		Game.TieredAchievement('部分の総和','<b>100</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',3);
		Game.TieredAchievement('熊は何度もやって来る','<b>150</b> 自己無限生成エンジンを所持する。<q>どこから?</q>','自己無限生成エンジン',4);
		Game.TieredAchievement('代り映えのない','<b>200</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',5);
		Game.TieredAchievement('最後の再帰','<b>250</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',6);
		Game.TieredAchievement('一から、多数','<b>300</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',7);
		Game.TieredAchievement('再帰の一例','<b>350</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',8);
		Game.TieredAchievement('この実績の詳細については、実績名を参照して下さい','<b>400</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',9);
		Game.TieredAchievement('僕はものすごくメタ、この実績さえも','<b>450</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',10);
		Game.TieredAchievement('全然飽きない','<b>500</b> 自己無限生成エンジンを所持する。','自己無限生成エンジン',11);
		...
		order=6000;
		new Game.Achievement('ルネサンスの焼き菓子職人','<b>400</b> 個のアップグレードと <b>4000</b> 個の施設を所持する。<q>よく目を凝らすと、それは巨人 - 絶滅して久しいと最近まで思われていた、高くそびえ立つ謎の人型種族 - の肩の上に立っている。</q>',[10,10]);
		...
		Game.CpsAchievement('ピークに達してさえいない');
		...
		order=61615;
		new Game.Achievement('新規公募','初めて在庫市場で利益を出す。',[0,33]);
		new Game.Achievement('ルーキーナンバー','在庫市場の全商品を <b>100個</b> 以上所持する。<q>Gotta pump those numbers up!</q>',[9,33]);
		new Game.Achievement('貧乏貴族','在庫市場の全商品を <b>500個</b> 以上所持する。<q>What kind of twisted individual is out there cramming camels through needle holes anyway?</q>',[10,33]);
		new Game.Achievement('倉庫いっぱい','在庫市場の全商品を <b>1000個</b> 以上所持する。',[11,33]);
		new Game.Achievement('良い1日だ','在庫市場で1度にCpS <b>1日分</b>($86,400) 売り上げる。',[1,33]);
		new Game.Achievement('買って買って買って','在庫市場で1度にCpS <b>1日分</b>($86,400) 買い入れる。',[1,33]);
		new Game.Achievement('当座資産','在庫市場の利益がCpS <b>丸1年分</b> を超える。',[12,33]);
		new Game.Achievement('ネズミ講','在庫市場の <b>最上級</b> の本社を解禁する。',[18,33]);
		
		order=10000;
		new Game.Achievement('ジェリクル','<b>10</b> 種類の猫を所持する。<q>Jellicles can and jellicles do! Make sure to wash your jellicles every day!</q>',[18,19]);
		
		order=7002;
		new Game.Achievement('550個記念','<b>全施設を550個</b> 以上所持する。<q>This won\'t fill the churning void inside, you know.</q>',[29,26]);
		
		Game.CpsAchievement('今まで何を食べていた');
		Game.CpsAchievement('大量消費');
		Game.CpsAchievement('まだ言うの?');
		Game.BankAchievement('大量に貯留');
		Game.BankAchievement('隙間なく詰め込まれた');
		Game.BankAchievement('すさまじく薄いだけだよ','ほんとにそれだけ!');
		
		order=1000;new Game.Achievement('カチカチ魂','クリックで <b>1,000,000,000,000,000,000,000,000,000</b> クッキー生産する。',[11,31]);
		order=1050;new Game.Achievement('尊敬するよ','<b>700</b> カーソル所持する。',[0,17]);
		order=1100;Game.TieredAchievement('大人は黙ってて','<b>600</b> グランマ所持する。','グランマ',13);
		order=1200;Game.TieredAchievement('熟しすぎ','<b>550</b> 農場所持する。','農場',12);
		order=1300;Game.TieredAchievement('いイワね','<b>550</b> 鉱山所持する。','鉱山',12);
		order=1400;Game.TieredAchievement('自分を自分でたたき上げ','<b>550</b> 工場所持する。','工場',12);
		order=1425;Game.TieredAchievement('イカした勘定','<b>550</b> 銀行所持する。','銀行',12);
		order=1450;Game.TieredAchievement('祈りを糧に','<b>550</b> 神殿所持する。','神殿',12);
		order=1475;Game.TieredAchievement('ヒギタスフィギタスミギタスムン','<b>550</b> 魔法使いの塔所持する。','魔法使いの塔',12);
		order=1500;Game.TieredAchievement('三匹荒野を行く','<b>550</b> 宇宙船所持する。','宇宙船',12);
		order=1600;Game.TieredAchievement('一時的なもの','<b>550</b> 錬金術室所持する。','錬金術室',12);
		order=1700;Game.TieredAchievement('行かせないでくれ','<b>550</b> ポータル所持する。','ポータル',12);
		order=1800;Game.TieredAchievement('原始人から宇宙へ','<b>550</b> タイムマシン所持する。','タイムマシン',12);
		order=1900;Game.TieredAchievement('固有味','<b>550</b> 反物質凝縮器所持する。','反物質凝縮器',12);
		order=2000;Game.TieredAchievement('光ほど軽い軽食','<b>550</b> プリズム所持する。','プリズム',12);
		order=2100;Game.TieredAchievement('運命に抗う','<b>550</b> チャンスメーカー所持する。','チャンスメーカー',12);
		order=2200;Game.TieredAchievement('同語反復','<b>550</b> 自己無限生成エンジン所持する。','自己無限生成エンジン',12);
		order=2300;Game.TieredAchievement('中括弧','<b>550</b> Javascriptコンソール所持する。<q>Or as the French call them, mustache boxes.<br>Go well with quotes.</q>','Javascriptコンソール',12);
		
		order=10000;
		new Game.Achievement('7つの蹄鉄','<b>ゴールデンクッキーを27777枚</b> クリックする。<q>Enough for one of those funky horses that graze near your factories.</q>',[21,33]);Game.last.pool='shadow';
		
		order=11000;
		new Game.Achievement('昔の日々','<b>忘れられたマドレーヌ</b>を見つける。<q>DashNet Farms remembers.</q>',[12,3]);
		...
		*/
		...
		new Game.buffType('building buff',function(time,pow,building)
		{
			...
			return {
				...
				desc:'所持している'+obj.amount+obj.plural+'がCpSを強化!<br>クッキーの生産が'+Game.sayTime(time*Game.fps,-1)+'、 +'+(Math.ceil(pow*100-100))+'% !',
		...
		new Game.buffType('building debuff',function(time,pow,building)
		{
			...
			return {
				...
				desc:'所持している'+obj.amount+obj.plural+'がCpSを鈍化!<br>クッキーの生産が'+Game.sayTime(time*Game.fps,-1)+'、 '+(Math.ceil(pow*100-100))+'% 遅く!',
		...
		{
			if (Game.specialTab=='dragon' && Game.dragonLevel>=4 && Game.Has('ドラゴン飼育') && l('specialPic'))
			{
				...
				if (Game.dragonLevel>=8 && Math.random()<1/20)
				{
					...
					var drops=['ドラゴンの鱗','ドラゴンの爪','ドラゴンの牙','ドラゴンのぬいぐるみ'];
					drops=shuffle(drops);
		...
		Game.ToggleSpecialMenu=function(on)
		{
			if (on)
			{
				...
				var str='<div id="specialPic" '+Game.clickStr+'="Game.ClickSpecialPic();" style="'+((Game.specialTab=='dragon' && Game.dragonLevel>=4 && Game.Has('ドラゴン飼育'))?'cursor:pointer;':'')+'position:absolute;left:-16px;top:-64px;width:96px;height:96px;background:url(img/'+pic+');background-position:'+(-frame*96)+'px 0px;filter:drop-shadow(0px 3px 2px #000);-webkit-filter:drop-shadow(0px 3px 2px #000);"></div>';
		...
		Game.OpenSesame=function()
		{
			...
			str+='<a class="option neato" '+Game.clickStr+'="Game.heralds=100;l(\'heraldsAmount\').innerHTML=Game.heralds;Game.externalDataLoaded=true;Game.recalculateGains=1;">紋章官最大</a>';
			str+='<div class="line"></div>';
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.Has('永久のハートのビスケット')) Game.Win('愛らしいクッキーたち');
				if (Game.season=='easter')
				{
					var eggs=0;
					for (var i in Game.easterEggs)
				...
				if (Game.handmadeCookies>=10000000000000000000000000) {Game.Win('もう…一度…クリックを…');Game.Unlock('プラズマ大理石のマウス');}
				if (Game.handmadeCookies>=1000000000000000000000000000) {Game.Win('カチカチ魂');Game.Unlock('奇石から生まれたマウス');}
				...
				if (buildingsOwned>=4000 && Game.UpgradesOwned>=400) Game.Win('ルネサンスの焼き菓子職人');
